...
2. 多根包含（$K_n$包含$K_{n-1}$到$K_{n-x}$, x>1）：
   - 当 $h_n > h_{n-x-1}$时：
     - $K_{n-1}$向上 → $K_n$向上，作向上包含处理
     - $K_{n-1}$向下 → $K_n$向上，作为独立K线处理（特殊K线）
   - 当 $l_n < l_{n-x-1}$时：
     - $K_{n-1}$向下 → $K_n$向下，作向下包含处理
     - $K_{n-1}$向上 → $K_n$向下，作为独立K线处理（特殊K线）
...
- $h_n > h_{n-x-1}$ → $K_n$向上，$h_(n-x)$到$h_n$，作向上合并为一个K线
- $l_n < l_{n-x-1}$ → $K_n$向下，$h_(n-x)$到$h_n$，作向下合并为一个K线
