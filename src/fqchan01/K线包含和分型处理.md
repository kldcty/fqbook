...
#### 初始方向确定（n=0时）
1. 查找最小的$x>0$使得$K_{n+x}$与$K_n$无包含关系
2. 方向判定：
   - 若 $h_{n+x} > h_n$ 且 $l_{n+x} > l_n$ → $K_n$方向向下
   - 若 $l_{n+x} < l_n$ 且 $h_{n+x} < h_n$ → $K_n$方向向上

#### 后续K线处理（n>0时）
假设$K_0$到$K_{n-1}$已处理，$K_n$为待处理K线：

##### 情况1：前包含关系（$K_n$包含$K_{n-1}$）
- 方向继承：
  - $K_{n-1}$向上 → $K_n$向上，做向上包含处理
  - $K_{n-1}$向下 → $K_n$向下，做向下包含处理

##### 情况2：后包含关系
1. 单根包含（$K_n$仅包含$K_{n-1}$）：
   - 这也是方向继承
   - $K_{n-1}$向上 → $K_n$向上，$K_n$和$K_{n-1}$作向上包含处理
   - $K_{n-1}$向下 → $K_n$向下，$K_n$和$K_{n-1}$作向下包含处理

2. 多根包含（$K_n$包含$K_{n-1}$到$K_{n-x}$, x>1）：
   - 当 $h_n > h_{n-x-1}$时：
     - $K_{n-1}$向上 → $K_n$向上，$K_n$和$K_{n-1}$作向上包含处理
     - $K_{n-1}$向下 → $K_n$向上，$K_n$作为独立K线处理（特殊K线）
   - 当 $l_n < l_{n-x-1}$时：
     - $K_{n-1}$向下 → $K_n$向下，$K_n$和$K_{n-1}$作向下包含处理
     - $K_{n-1}$向上 → $K_n$向下，$K_n$作为独立K线处理（特殊K线）
...
如果$K_n$后包含$K_{n-1}$到$K_{n-x}$, x>1：
- 当 $h_n > h_{n-x-1}$时：
  - $K_{n-1}$向上 → $K_n$向上，$K_n$和$K_{n-1}$作向上包含处理
  - $K_{n-1}$向下 → $K_n$向上，$K_n$到$K_{n-x}$做向下包含处理，原$K_{n-x}$到$K_{n-1}$的包含K线取消（特殊K线）
- 当 $l_n < l_{n-x-1}$时：
  - $K_{n-1}$向下 → $K_n$向下，$K_n$和$K_{n-1}$作向下包含处理
  - $K_{n-1}$向上 → $K_n$向下，$K_n$到$K_{n-x}$做向上包含处理，原$K_{n-x}$到$K_{n-1}$的包含K线取消（特殊K线）
