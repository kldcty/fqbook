...
很显然，在这个定义中，如果n=0那么是不是就玩不下去了，所以我们这里要精确定义出起始K线的方向处理作为一种补充。这里我们给出一个完整的定义：
...
>
> 当$n>0$时，方向确定算法如下：
> $K_0$到$K_n$为处理过包含关系后的K线，$K_{n+1}$为下一个要处理的K线，假设$K_n$和$K_{n+1}$存在包含关系，而$K_n$和$K_{n-1}$不存在包含关系，那么：
> - 若 $h_n > h_{n-1}$，则称$K_{n-1}$、$K_n$、$K_{n+1}$是向上的，$K_{n+1}$要做向上包含处理
> - 若 $l_n < l_{n-1}$，则称$K_{n-1}$、$K_n$、$K_{n+1}$是向下的，$K_{n+1}$要做向下包含处理
